...
Please let me know the next steps.`;
  ...
  const handleSelect = useCallback((size) => {
    ...
    if (customProductDetails?.hasVariations) {
      const selectedVariation = customProductDetails.variations.find(
      ...
      setDisplayPrice({
        regularPrice: customProductDetails?.regularPrice?.$numberDecimal,
        salePrice: customProductDetails?.salePrice?.$numberDecimal,
        discount: customProductDetails?.discount?.$numberDecimal,
      });
    }
  }, [customProductDetails]);
  ...
  }, [dispatch, openCart, navigate, userId]);
